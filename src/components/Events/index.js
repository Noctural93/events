...
class Events extends Component {
  ...
  getActiveEventStatus = () => {
    ...
    const activeEventDetails = eventsList.find(
      eachItem => activeEvent === eachItem.id,
    )
  ...
  updateActiveEvent = id => {
    this.setState({activeEvent: id, isButtonClicked: true})
  }

  render() {
    const {isButtonClicked, activeEvent} = this.state
    return (
          ...
          <ul className="events-list">
            {eventsList.map(eachItem => (
              <EventItem
                ...
                updateActiveEvent={this.updateActiveEvent}
